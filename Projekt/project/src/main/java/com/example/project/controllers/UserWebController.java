...
public class UserWebController {
    ...
    public String update(@Valid @ModelAttribute("userInstance") UserDTO userDTO,
        ...
        if (bindingResult.hasErrors()) {
            ...
            if (userService.updateUser(userDTO) != null)
                atts.addFlashAttribute("message", "Aktualizacja użytkownika przebiegła pomyślnie");
            else
                atts.addFlashAttribute("message", "Aktualizacja użytkownika nie powiodła się.");
    ...
    public String save(@Valid @ModelAttribute("userInstance") UserDTO userDTO,
        ...
        if (bindingResult.hasErrors()) {
            ...
            if (userService.createNewUser(userDTO) != null)
                atts.addFlashAttribute("message", "Użytkownik został pomyślnie utworzony");
            else
                atts.addFlashAttribute("message", "Tworzenie użytkownika nie powiodło się.");
    ...
    public String delete(@RequestParam Long id, RedirectAttributes atts) {
        User userInstance = userRepository.findById(id)
            .orElseThrow(() -> new IllegalArgumentException("Nie odnaleziono użytkownika:" + id));

        userRepository.delete(userInstance);

        atts.addFlashAttribute("message", "Usunięto użytkownika.");
